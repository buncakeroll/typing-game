...
const data = require("../../src/data-new.json");
...
class Text extends React.Component {
  ...
  displayTimeElapsed() {
    ...
    if (this.state.timeElapsed == 60) {
      ...
      alert(`Your word's per minute is ${this.state.wpm}`);
  ...
  render() {
    ...
    if (this.index < this.textArray.length) correct += " ";
    let rest =
      this.index <= paragraph.length
        ? " " + paragraph.slice(this.index + 1).join(" ")
        : "";
    ...
    return (
      <div id="text-excerpt">
        ...
        <div className="results">
          <p id="time-elapsed">
            Timer
            <span className="box">{this.state.displayTime}</span>
          </p>
          <p id="wpm">
            Per Minute
            <span className="box">{this.state.wpm}</span>
          </p>
        </div>
      </div>
    );
